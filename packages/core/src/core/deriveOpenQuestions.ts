 ...
 *
 * Hierarchy: Beat ← PlotPoint.alignedBeatId ← PlotPoint ← REALIZED_BY ← Scene
...
function deriveBeatUnrealized(graph: GraphState): OpenQuestion[] {
  ...
  // Build Beat → PlotPoint IDs map from alignedBeatId property
  const plotPointsByBeat = new Map<string, string[]>();
  for (const node of graph.nodes.values()) {
    if (node.type === 'PlotPoint' && (node as any).alignedBeatId) {
      const beatId = (node as any).alignedBeatId as string;
      const existing = plotPointsByBeat.get(beatId) || [];
      existing.push(node.id);
      plotPointsByBeat.set(beatId, existing);
    }
  }
  ...
  for (const beat of beats) {
    ...
    const plotPointIds = plotPointsByBeat.get(beat.id) || [];
 ...
 *
 * Uses hierarchy: Beat ← PlotPoint.alignedBeatId ← PlotPoint ← REALIZED_BY ← Scene
 */
