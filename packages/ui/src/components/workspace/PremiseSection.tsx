/**
 * PremiseSection - Premise content wrapped in CollapsibleSection.
 * Displays logline, genre/tone, and setting with inline editing.
 */
...
export function PremiseSection() {
  ...
  const collapsedSummary = useMemo(() => {
    const parts: string[] = [];
    if (loglineText) {
      const truncated = loglineText.length > 60 ? loglineText.slice(0, 60) + '...' : loglineText;
      parts.push(`"${truncated}"`);
    }
    if (genreToneDisplay) {
      parts.push(`Genre: ${genreToneDisplay}`);
    }
    if (settingDisplay) {
      parts.push(`Setting: ${settingDisplay}`);
    }
    return parts.length > 0 ? parts.join(' | ') : 'No premise data yet';
  }, [loglineText, genreToneDisplay, settingDisplay]);

  // Get badge counts
  const premiseCounts = sectionChangeCounts?.premise;
  const badge = premiseCounts && (premiseCounts.additions > 0 || premiseCounts.modifications > 0)
    ? { additions: premiseCounts.additions, modifications: premiseCounts.modifications }
    : undefined;
